...
当一个领域的知识量很大时，一个 .md 文件装不下。这时候可以用**目录结构**：

```
remotion-best-practices/        ← 一个技能
├── SKILL.md                    ← 主入口（目录 + 使用说明）
└── rules/                      ← 34 个子文档
...
SKILL.md 像一本书的目录，rules/ 里的文件是各章节。AI 需要哪个主题就去读哪个章节，不用一次性全部加载。

### 不会写 Markdown 怎么办？

即使是 SKILL.md 这种"最简单"的方式，对普通人来说也有门槛——要写 YAML 头部、用 Markdown 格式组织内容。

所以 AgentOS 计划支持 **AI 自动生成技能**：

```
你只需要用大白话描述：
  "我想要一个技能，帮我写正式的商务邮件，要有称呼、正文、结尾"

AI 自动帮你生成完整的 SKILL.md → 注册为技能 → 你直接用
```

这样，创建技能的门槛从"会写 Markdown"变成了"会说话就行"。
...
以上所有技术概念——MCP、HTTP Skill、SKILL.md、内置 Skill——都是后台实现细节。作为普通用户，你只需要做两件事：

**第一，找技能**：打开技能库，看到需要的技能，点"安装"。

**第二，用技能**：告诉 AI 你想做什么，它会自己找到对的技能去完成。

甚至，如果技能库里没有你想要的，你可以直接告诉 AI"我想要一个什么样的技能"，AI 会帮你创建一个。

你不需要知道"天气查询"背后是内置代码还是 MCP。你不需要知道"GitHub 工具"走的是什么协议。你不需要知道"写作助手"是一个 Markdown 文件。

**你只需要说出你的需求。技术的事情，交给平台。**
